...
class VersionHelper {
		...
		foreach($entity->commits as $commit) {
			...
			foreach(self::$types as $tName => $tValue) {
				foreach($tValue as $exp) {
					$exp = '#' . $exp . '#i';
					if(preg_match($exp, $commit->message) && isset($type[$tName])) {
	...
	private static function getVersionVariations($versionList) {
		...
		//$result['current'] = \PhpLab\Core\Legacy\Yii\Helpers\ArrayHelper::last($versionList);
		return $result;
	}
	...
	private static function getLastFromTree($tree) {
		...
		return \PhpLab\Core\Legacy\Yii\Helpers\ArrayHelper::last($versions);
	}
