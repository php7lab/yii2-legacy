...
$columns = [
    [
        ...
        'value' => function (\yubundle\reference\domain\entities\ItemEntity $data) {
            $bookEntity = App::$domain->reference->book->repository->oneById($data->book_id);
            if ($bookEntity->levels != 1) {
                return
                    Html::a(
                        $data->value,
                        ['/reference/item/index', 'book_id' => $data->book_id, 'parent_id' => $data->id]
                    );
            } else {
                return Html::a(
                    $data->value,
...
//$bookId = Yii::$app->request->getQueryParam('book_id');
...
if ($companyId == $bookEntity->owner_id) {
    echo Html::a(Yii::t('action', 'create'), [
        $baseUrl . 'create',
        'book_id' => Yii::$app->request->getQueryParam('book_id'),
